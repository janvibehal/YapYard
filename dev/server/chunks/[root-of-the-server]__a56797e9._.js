...
"[project]/utils/cloudinary.js [app-route] (ecmascript)", ((__turbopack_context__) => {
"use strict";
...
var __TURBOPACK__imported__module__$5b$project$5d2f$utils$2f$cloudinary$2e$js__$5b$app$2d$route$5d$__$28$ecmascript$29$__ = __turbopack_context__.i("[project]/utils/cloudinary.js [app-route] (ecmascript)");
...
// Define the valid types based on your Mongoose schema enum
...
async function POST(req) {
    try {
        ...
        // CRITICAL: Use the correct keys ('mediaFile') and getAll() for arrays
        const text = formData.get("text") || "";
        const mediaFiles = formData.getAll("mediaFile");
        const mediaTypes = formData.getAll("mediaType");
        const media = [];
        // 2. PROCESS MULTIPLE MEDIA FILES
        if (mediaFiles.length > 0) {
            for(let i = 0; i < mediaFiles.length; i++){
                // Note: formData.getAll returns an array of File objects or strings.
                const file = mediaFiles[i];
                const rawMediaType = mediaTypes[i];
                // Validate media type (CRITICAL FIX)
                const mediaType = rawMediaType && VALID_MEDIA_TYPES.includes(rawMediaType.toString().toLowerCase()) ? rawMediaType.toString().toLowerCase() : null;
                // Check if item is a valid File object AND the mediaType is valid
                // Check file.name as well, as some browsers send an empty File object if input is cancelled
                if (file instanceof File && file.size > 0 && file.name && mediaType) {
                    const arrayBuffer = await file.arrayBuffer();
                    const buffer = Buffer.from(arrayBuffer);
                    const uploadResponse = await new Promise((resolve, reject)=>{
                        const uploadStream = __TURBOPACK__imported__module__$5b$project$5d2f$utils$2f$cloudinary$2e$js__$5b$app$2d$route$5d$__$28$ecmascript$29$__["default"].uploader.upload_stream({
                            // Use the explicitly validated mediaType for resource_type
        ...
        const newPost = await __TURBOPACK__imported__module__$5b$project$5d2f$models$2f$Post$2e$js__$5b$app$2d$route$5d$__$28$ecmascript$29$__["default"].create({
            author: decoded.id,
            text,
            media
        });
...
async function GET() {
    try {
        await (0, __TURBOPACK__imported__module__$5b$project$5d2f$lib$2f$dbConnect$2e$js__$5b$app$2d$route$5d$__$28$ecmascript$29$__["default"])();
        const posts = await __TURBOPACK__imported__module__$5b$project$5d2f$models$2f$Post$2e$js__$5b$app$2d$route$5d$__$28$ecmascript$29$__["default"].find().populate("author", "name avatarUrl").sort({
            createdAt: -1
        });
...
//# sourceMappingURL=%5Broot-of-the-server%5D__a56797e9._.js.map